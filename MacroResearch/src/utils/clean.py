...
def tidy_dataframe(df: pd.DataFrame, indicator_name: str) -> pd.DataFrame:
    ...
    if "FREQ" in df.columns:
        ...
        freq_column_name = "FREQUENCY"
        df["FREQUENCY"] = pd.infer_freq(df["TIME_PERIOD"])

    df = df[["TIME_PERIOD", freq_column_name, "INDICATOR", "value"]]
...
def flag_anomalies(df: pd.DataFrame, value_col: str = "value") -> pd.DataFrame:
    ...
    df = df.sort_values(["INDICATOR", "TIME_PERIOD"])

    # Percent change
    df["pct_change"] = df.groupby(["INDICATOR"])[value_col].pct_change(fill_method=None).abs()
